...
const mean = (array) =>
  Math.round(
    (array.reduce((a, b) => a + b) / array.length + Number.EPSILON) * 100
  ) / 100;
...
async function updateEduData(data) {
  var students = await Promise.all(
    data.students.map(async (student) => {
      const response = await fetch(
        "https://de.edu.orb.ru/er/index/report/report/progress/participant_marks?" +
          new URLSearchParams({
            GRADEFK: data.gid,
            PARTICIPANTFK: student.sid,
            DATE_BEGIN: periods.year[0] + ".2025",
            DATE_END: periods.year[1] + ".2026",
          }).toString(),
        {
...
async function parseEduData(gid) {
  const response = await fetch(
    "https://de.edu.orb.ru/er/index/lookup/PARTICIPANTFK?" +
      new URLSearchParams({ parent: gid }).toString(),
    {
      headers: { Cookie: process.env.COOKIE },
      method: "GET",
    }
...
class Statistics {
  ...
  getStatistics() {
    ...
    var ratings = {};
    Object.entries(this.getRating(this.edu_data.students)).forEach(
      ([name, rating]) => {
        ratings[name] = { grade: rating };
      }
    );
    Object.entries(
      this.getRating([this.edu_data.students.find((x) => x.sid == this.sid)])
    ).forEach(([name, rating]) => {
    ...
    var students = this.edu_data.students.map((student) => ({
      rating: Object.values(this.getRating([student])).reduce(
        (acc, a) => ({
    ...
    const averageRating = round(
      ratingsCopy
        .map(([name, rating]) => rating.relative.average)
        .reduce((a, b) => a + b)
    );
    const countRating = round(
      ratingsCopy
        .map(([name, rating]) => rating.relative.count)
        .reduce((a, b) => a + b)
    );
